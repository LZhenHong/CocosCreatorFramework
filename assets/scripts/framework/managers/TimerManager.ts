...
export default class TimerManager extends BaseManager {
    private _timer: cc.Component | null = null;
    ...
    public setupTimer(timer: cc.Component) {
        this._timer = timer;
    }
    ...
    public loopCallUntil(delayDuration: number, loopInterval: number, callback: Function, stopFunction: (count: number) => boolean): number {
        ...
        if (this._timer) {
            ...
            this._callbackMap.set(handler, func);
            this._timer.schedule(func, loopInterval, cc.macro.REPEAT_FOREVER, delayDuration);
            return handler;
        }
        return 0;
    }
    ...
    public loopCall(delayDuration: number, loopInterval: number, callback: Function): number {
        ...
        if (this._timer) {
            ...
            this._timer.schedule(callback, loopInterval, cc.macro.REPEAT_FOREVER, delayDuration);
            return handler;
        }
        return 0;
    }
